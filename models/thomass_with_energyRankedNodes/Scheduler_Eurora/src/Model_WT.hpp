...
class Model21: public IModelAdvanced
{
private:
...
public:
	Model21();
	Model21(QueueArray queue,NodeArray nodes,JobArray jobs){_queue=queue,_nodes=nodes,_jobs=jobs;_minTime=1;_os=-1;_wt=-1;_wtDelta=-1;_nl=-1;_nlDelta=-1;_mk=-1;_mkDelta=-1;}
	JobArray solve(int refTime);
	double getOptimalSolution(){return _os;}
};

inline JobArray Model21::solve(int refTime)
{
	bool solved = false;
	bool retry=false;
